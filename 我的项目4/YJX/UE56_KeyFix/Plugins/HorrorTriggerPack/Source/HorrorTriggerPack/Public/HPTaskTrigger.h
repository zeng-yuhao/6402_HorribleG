...
public:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Task Trigger|Requirements",
        meta=(DisplayName="Require BP Key / 需要 BP_Key 钥匙"))
    bool bRequireBPKey = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Task Trigger|Requirements",
        meta=(DisplayName="Consume Key On Submit / 提交时消耗钥匙"))
    bool bConsumeKeyOnSubmit = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Task Trigger|Requirements",
        meta=(DisplayName="Required Key ID / 需要的钥匙ID"))
    FName RequiredKeyId = TEXT("YYN_Key");
