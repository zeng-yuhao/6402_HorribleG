...
u.get_editor_subsystem(u.LevelEditorSubsystem).load_level('/Game/HorrorWhitebox/Maps/L_Horror_Floorplan_Whitebox1')
actors=u.get_editor_subsystem(u.EditorActorSubsystem).get_all_level_actors()
obstacles=[]
for a in actors:
    if isinstance(a,u.StaticMeshActor) and (str(a.get_folder_path())=='01_Walls' or (str(a.get_folder_path()).startswith('Asylum/Rooms') and str(a.static_mesh_component.get_collision_profile_name())=='BlockAll')):
...
Path('/Users/panjiang/Documents/Unreal Projects/Art_Tech_UE5_6/Scripts/asylum_validation.json').write_text(json.dumps(result,indent=2))
u.log('ASYLUM_VERIFY_SUCCESS '+json.dumps(result))
